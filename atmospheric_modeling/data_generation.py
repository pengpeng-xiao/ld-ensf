...
def run_ic_file(ic_file, output_dir, h_f0_value, sigma):
    # Extract IC file name without extension
    # output_dir = output_dir # + f"hf0_{h_f0_value}" + ic_file.split("/")[-1].split(".")[0] 
    run_prefix = f"{ic_file.split('/')[-1].split('.')[0]}_hf0_{h_f0_value}_sigma_{sigma}".replace(".", "p")
    output_dir = Path(output_dir) / run_prefix
    ...
    nu = 1e5 * meter**2 / second / (32) ** 2  # Hyperdiffusion matched at ell=96
    g = 9.80616 * meter / second**2
    timestep = 60 * second
    burn_in = 0.25 * year
    # stop_sim_time = burn_in + 3 * year  # 1*year
    stop_sim_time = burn_in + 0.5 * year  # 1*year
    dtype = np.float64
    ...
    theta_var["g"] += lat
    h_f0 = h_f0_value * meter  # Using the parameter value
    ...
    snapshots = solver.evaluator.add_file_handler(
        ...
        output_dir, sim_dt=4 * hour, max_writes=1 * year
    )
    ...
    while solver.proceed:
        ...
        if (solver.iteration - 1) % 10 == 0:
            ...
            # print(np.mean(np.abs(h.allgather_data(layout = "g"))))
...
if __name__ == "__main__":
    ...
    run_ic_file(all_files[ind], output_dir, args.hf0, args.sigma)