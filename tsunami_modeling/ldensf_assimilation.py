...
def main(opt):
    ...
    for true_traj in range(10):
        ...
        assimilated_states = model.forward_data_assimilation_ensf(data=data_test, observations=observation, encoder=encoder, noise_level=noise_level, 
                                                                device=opt.device, optimal_latent=None, scaling = scaling, obs_sigma =  obs_sigma, eps_alpha = eps_alpha, euler_steps =euler_steps).detach().cpu().numpy()
        # denormalize
        ...
        print(f"RMSE state: ",rmse_state.shape)
        # utils.set_seed(opt.seed)  
        ...
        assimilated_latent, assimilated_u, encoded_latent = model.forward_data_assimilation_ensf(data=data_test, observations=observation, encoder=encoder, noise_level=noise_level, 
                                                                              device=opt.device, latent_state = True, optimal_latent=None, scaling = scaling, obs_sigma =  obs_sigma, eps_alpha = eps_alpha, euler_steps =euler_steps)
    ...
    wandb.log(
        {"state": wandb.plot.line_series(xs=np.arange(50)*40, 
        ...
        "latent": wandb.plot.line_series(xs=np.arange(50)*40,
        ...
        "u": wandb.plot.line_series(xs=np.arange(50)*40,
