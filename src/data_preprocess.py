...
def select_fixed_subset(data, indices):
    assert len(data["x"].shape) == 4 
    assert len(data["y"].shape) == 4 
...
class DataPreprocessorOrder():
    ...
    def __init__(
        ...
        indices = np.arange(self.Ni)
        train_and_valid_indices = indices[:train_size+valid_size]  # 前 train_size 作为 train+valid
        self.test_indices = indices[train_size+ valid_size : train_size+valid_size + test_size]  # 后 test_size 作为 test

        permuted = np.random.permutation(train_and_valid_indices)
        self.valid_indices = permuted[:valid_size]
        self.train_indices = permuted[valid_size:]
