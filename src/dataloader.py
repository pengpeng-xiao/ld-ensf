...
class BranchDataset(Dataset):
    ...
    def __getitem__(self, idx):
        new_data = {
            ...
            "dt": torch.tensor(self.data["dt"]).unsqueeze(0)
...
class ICDataset(Dataset):
    ...
    def __getitem__(self, idx):
        new_data = {
            "ic": torch.from_numpy(self.data["ic"][idx]),
            "x": torch.from_numpy(self.data["x"][idx]),
            "y": torch.from_numpy(self.data["y"][idx]),
            "dt": torch.tensor(self.data["dt"])
        }
        return new_data
    
    def collate_fn(batch):
        # Your custom collation logic
        ic_stack = torch.stack([item["ic"] for item in batch], dim=0)
        x_stack = torch.stack([item["x"] for item in batch], dim=0)
        y_stack = torch.stack([item["y"] for item in batch], dim=0)
        dt_stack = batch[0]["dt"]
        return {
