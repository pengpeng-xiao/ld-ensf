...
class Normalize_gaussian():
    ...
    def normalize_forw_bchw(self, v):
        normalized = (v - self.mean[np.newaxis, :, np.newaxis, np.newaxis]) / self.std[np.newaxis, :, np.newaxis, np.newaxis]
        return normalized

    def normalize_back_bchw(self, v):
        denomalized = v * self.std[np.newaxis, :, np.newaxis, np.newaxis] + self.mean[np.newaxis, :, np.newaxis, np.newaxis]
        return denomalized